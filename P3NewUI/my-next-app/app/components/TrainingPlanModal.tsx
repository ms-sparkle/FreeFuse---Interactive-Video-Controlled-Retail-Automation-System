  ...
  return (
    <div ref={ref} className="relative w-full">
      <button
        ...
        className="w-full flex items-center justify-between bg-slate-800 border border-slate-700 rounded-lg px-3 py-2.5 text-sm text-left focus:outline-none focus:border-cyan-500 hover:border-slate-500 transition-colors"
      >
            ...
            {filtered.map((item) => (
              <li key={item.name}>
                <button
                  ...
                  className={`w-full text-left px-3 py-2.5 text-sm hover:bg-slate-700 transition-colors border-b border-slate-700/50 last:border-0
                    ${value?.name === item.name ? "text-cyan-400 bg-slate-700/60" : "text-slate-200"}`}
                >
                  <span className="font-medium">{item.name}</span>
                  <span className="text-slate-500 text-xs ml-2">· {item.type}</span>
                  <div className="text-[10px] text-slate-500 mt-0.5">{item.primary_nodes.join(", ")}</div>
                </button>
              </li>
            ))}
          </ul>
    ...
    <div className="rounded-xl border border-slate-700 bg-slate-800/60 p-4 mt-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <span className="inline-block mt-1 px-2 py-0.5 rounded-full text-[10px] font-semibold border border-cyan-500/40 bg-cyan-500/10 text-cyan-300">
            {workout.type}
          </span>
        </div>
        <Dumbbell size={18} className="text-cyan-400 shrink-0 mt-0.5" />
      </div>
      <div className="mt-3 space-y-1.5">
        <div>
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Primary muscles</p>
          <div className="flex flex-wrap gap-1">
            {workout.primary_nodes.map((m) => (
              <span key={m} className="px-2 py-0.5 rounded bg-slate-700 text-xs text-slate-300">{m}</span>
            ))}
          </div>
        </div>
        {workout.secondary_nodes.length > 0 && (
          <div>
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Secondary muscles</p>
            <div className="flex flex-wrap gap-1">
              {workout.secondary_nodes.map((m) => (
                <span key={m} className="px-2 py-0.5 rounded bg-slate-700/60 text-xs text-slate-400">{m}</span>
              ))}
            </div>
          </div>
        )}
      ...
      <div className="bg-slate-900 border border-slate-800 w-full max-w-md rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="p-5 border-t border-slate-800 flex gap-3">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 py-2.5 rounded-xl border border-slate-700 text-slate-300 hover:text-white hover:border-slate-500 transition-colors text-sm font-medium"
          >
            Cancel
          </button>
          <button
...
export function WorkoutSuggestionCard({
  ...
  function handleSuccess(_name: string) {
    // Could trigger a toast or parent refresh here if needed
  }

  return (
    <>
      <div
        className="relative flex items-center gap-3 text-white bg-slate-800 p-3 rounded border-l-4 border-cyan-500 group overflow-hidden"
      ...
      >
        ...
        <div className={`flex items-center gap-3 flex-1 transition-opacity duration-150 ${hovered ? "opacity-30" : "opacity-100"}`}>
          <div className="flex-1">
            <div className="font-bold text-cyan-400">{workout.WorkoutName}</div>
        ...
        {hovered && (
          <div className="absolute inset-0 flex items-center justify-center gap-2 animate-in fade-in duration-150">
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-cyan-600 hover:bg-cyan-500 text-black text-xs font-bold transition-colors shadow-lg"
            >
              <Plus size={13} />
              Add workout
            </button>
